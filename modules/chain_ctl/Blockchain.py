...
def validate_keys(chain: dict):
    ...
    for i in chain:
        block_key = chain.get(i)["previous_hash"]
        prev_block = chain.get(block_key)
        encoded_block = json.dumps(prev_block).encode()
        hashed_block = shifter_(
            "".join(("0x", hashlib.sha256(encoded_block).hexdigest()))
        )
...
def update_user(txn_splits: dict = None, inv_splits: dict = None):
    wallet_keys = []
    wallets_master = {}
    try:
...
class Blockchain_:
    ...
    def load_chain_json(self) -> dict:
        ...
        try:
            with open(
                f"{os.getcwd()}/chain_data/Chain_state_{self.chain_id}.json", "r"
            ) as file:
            ...
            try:
                ...
                try:
                    with open(
                        f"{os.getcwd()}/chain_data/Chain_state_{self.chain_id}.json",
                        "x",
                    ) as file:
    ...
    def load_chain_backup(self) -> dict:
        ...
        try:
            with open(
                f"{os.getcwd()}/user_data/journal/Chain_{self.chain_id}_backup.json",
                "r",
            ) as file:
                chain_ = dict(json.load(file))
                return chain_
    ...
    def write_chain_json(self):
        ...
        """
        with open(
            f"{os.getcwd()}/chain_data/Chain_state_{self.chain_id}.json", "w"
        ) as file:
            file.write(json.dumps(self.chain, indent=2))
        with open(
            f"{os.getcwd()}/chain_data/Chain_data_{self.chain_id}.json", "w"
        ) as file:
    ...
    def check_previous_block(self, block_hash: str):
        """
        Not even a thing yet, don't worry about comments
        """
        pass
    ...
    def append_block_(self, block: Block_):
        ...
        self.validate_chain()
        if appendage.get("previous_hash") == self.get_tallest_block()[
            1
    ...
    def load_master_backup(self) -> dict:
        ...
        try:
            with open(
                f"{os.getcwd()}/user_data/journal/Master_backup.json", "r"
            ) as file:
                MASTER_CHAIN = dict(json.load(file))
                return MASTER_CHAIN
        except json.JSONDecodeError:
    ...
    def user_journal_update(self):
        ...
        try:
            try:
                with open(
                    f"{os.getcwd()}/user_data/journal/Master_backup.json", "x"
                ) as file:
                ...
                try:
                    with open(
                        f"{os.getcwd()}/user_data/journal/Master_backup.json", "w"
                    ) as file:
                        master_chain = json.dumps(self.load_master_chain(), indent=2)
                        file.write(master_chain)
                except FileNotFoundError:
    ...
    def join_data(self, wallet_: dict = None):
        ...
        for key, value in joined_txns_all.items():
            ...
            joined_txn_splits.update(
                {txn_data["to"]: sum([to_bal, float(txn_data["amt"])])}
            )
            joined_txn_splits.update(
                {txn_data["from"]: sum([from_bal, -float(txn_data["amt"])])}
            )
        ...
        update_user(joined_txn_splits, joined_invent_splits)
        return (
            joined_chain_data,
            joined_txns_all,
            joined_txn_splits,
